...
png(filename="Figure S1.png",width=16,height=24,units="cm",res=300)
...
# Formulate subset models for each combination of predictors 
        ...
        mod.full <- glmer(val~ pa * es.c + (1|country), family=Gamma(link = "log"))
        
        # Full model only with additive terms
        mod.add <- glmer(val~ pa + es.c + (1|country), family=Gamma(link = "log"))
        
        # Model with only protected status
        mod.pa <- glmer(val~ pa  + (1|country), family=Gamma(link = "log"))
        
        # Model with only ecystem service type
...
# Label the axis with ecosystem sevice types
...
# Add a legens
legend ("right", pch=c(15,15,15,15,15,15,0,15), col=c(col.ramp[c(5,2,4,6,3,1)],"black","black"),cex=1.2,
